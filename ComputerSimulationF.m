...
nSim=str2double(handles.SimText.String); % Number of simulations to be run
handles.hideBox.Visible='on'; % Hides axes while calculations are being done
win=zeros(1,nSim);
health=zeros(1,nSim);
...
for iSim=1:nSim
    ...
    %% Set Parameters
    load('stopdata')
    if stop % End Button is pressed
        handles.hideBox.Visible='off'; % Allows end screen to be displayed
        cla(handles.HBar); % Clears axes before image is displayed
        cla(handles.ProgressAxes);
        cla(handles.PlotAxes);
        handles.statPanel.Visible='off'; % Hides things in front of axes
        ...
        opening = imread('Opener.jpg'); % Displays opening image
    ...
    t=linspace(0,Z*length(x),length(x)); % Delta t is 60*Z minutes
    save('timedata','t','Z')
    
    [healthBar,tf,alive]=HealthBar(x,y,alive); % tf is index of t, not actual time
    
    if nSim>1 % Calculate stats if number of simulations is more than one
...
%% Plot
handles.hideBox.Visible='off'; % Hides things in front of axes

if nSim==1 && ~stop
    %% Plot Walk and Healthbar
    handles.HText.Visible='on'; % Makes necessarry things visible
    ...
    for i=1:tf
        ...
        % Determines axes
        if y(i)<10*stepLength % Axis displayed is always 20*stepLength
            axmin=0;
            axmax=20*stepLength;
        elseif ymax-y(i)<10*stepLength
            ...
            axmin=y(i)-10*stepLength; % Axis moves with player
        ...
        if stop % End Button pressed
            opening = imread('Opener.jpg'); % Display opener image
            openerImage = image('CData', opening);
        ...
        handles.TimeText.String=['Time: ',num2str(round(t(i),2)),' hr'];
    end
    
    % After Game Ends
    cla(handles.PlotAxes) % Clears axis
    handles.GoButton.Value=0; % Raises Go Button
    handles.TimeText.Visible='off'; % Hides other axes
    healthBar=0;
    bar(healthBar,'r','Parent',har)
    cla(har)
    handles.HBar.Visible='off'; % Hides axes that don't have anything to display
    handles.HText.Visible='off';
    cla(pax)
    axis(pax,'off')
    if alive && ~handles.EndButton.Value % Displays win image
        wPercent = imread('WinClose.jpg');
        winImage = image('CData', wPercent,'Parent',hax);
        winImage.XData = [0,width];
        winImage.YData = [axmax,axmin];
    elseif ~handles.EndButton.Value % Displays lose image
        lose = imread('LoseClose.jpg');
        loseImage = image('CData', lose,'Parent',hax);
        loseImage.XData = [0,width];
        loseImage.YData = [axmax,axmin];
    else
        opening = imread('Opener.jpg'); % Displays opener image
...
elseif ~stop
    ...
    % Makes bar graphs of statistics
    ...
    % Displays stats in appropriate text boxes
    ...
    % Determines mode and items used
    item1='None';
    item2='';
    ...
    % Displays mode and items at top of screen
    ...
    handles.GoButton.Value=0; % Lifts up Go Button
end