...
%   Outputs are used alter on in ComputerSimulationF. Gets most variables
%   from load function.

%% Initialize Vectors

% Length of vectors is estimate, preallocated for speed. Made correct length later.
x=zeros(1,250);
y=zeros(1,250);
zombie=zeros(1,250);
player=cell(1,250);
player(:)={0};
direction=cell(1,250);
direction(:)={0};
        ...
        if Radiation.yCenter(nr)-Radiation.rOuter(nr)<LeftWall.y(nw) ||...
            ...
            if CenterWall.left(nw)>0 && CenterWall.right(nw)<width
                ...
                if CenterWall.left<=0
                    spaces.one.left=LeftWall.right(nw);
                    % If Center Wall is acting as left
                    ...
                    % If Center Wall is acting as right
                    if CenterWall.right(nw)==LeftWall.right(nw)
                        spaces.one.right=RightWall.left(nw);
                    else
                        spaces.one.right=CenterWall.left(nw);
